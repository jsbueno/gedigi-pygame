...
def default_font():
    #FIXME: Load from the directory where the module is isntalled
    return pygame.font.Font ("VeraSeBd.ttf", 25)
...
class Editor(KeyboardMaps):
    ...
    def delete(self, position, n=1):
        for i in range(n):
            self.text.delete(position)

    def update(self):
        original_clip = self.screen.get_clip()
        self.screen.set_clip(self.clip)
        if self.background is not None:
            pygame.draw.rect(self.screen, self.background , self.clip)
        else:
            pygame.screen.fill((0,0,0,0))
    ...
    def apply_char_effects(self, linecoords):
        ...
        for pos, char in effected_chars.items():
            result = char.update()
            # FIXME: Check why dow e get this attributeerror sometimes
            try:
